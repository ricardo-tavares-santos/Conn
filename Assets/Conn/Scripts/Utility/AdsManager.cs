...
public class AdsManager : MonoBehaviour
{
	...
	private void AdMobAdvertisment (AdPackage.AdEvent adEvent)
	{
		...
		} /*else if (adEvent.type == AdPackage.AdEvent.Type.NativeExpress) {
			if (string.IsNullOrEmpty (admob.androidNativeExpressAdUnitID) && string.IsNullOrEmpty (admob.IOSNativeExpressAdUnitID)) {
				Debug.LogWarning ("NativeExpress AdUnit is not defined in AdMob component");
				return;
			}
			//Show NativeExpress Advertisment
			admob.RequestNativeExpressAdView (adEvent.adPostion);
		}*/
		#endif
	}
